...
const buildFullAddress = (data: AgentFormData): string => {
  return [data.street, data.street_number, data.neighborhood, data.city, data.state]
    .filter(Boolean)
    .join(', ');
};
  ...
  const onSubmit = async (data: AgentFormData) => {
    ...
    try {
      ...
      const { error } = await supabase.from('agents').insert({
        ...
        // Legacy composed field (used for backward compat / geocoding display)
                ...
                <div className="grid grid-cols-3 gap-3">
                  ...
                  <FormField control={form.control} name="state" render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl><Input placeholder="SP" maxLength={2} className="uppercase" {...field} onChange={e => field.onChange(e.target.value.toUpperCase())} /></FormControl>
                      <FormMessage />
                    </FormItem>
                  )} />
                </div>
